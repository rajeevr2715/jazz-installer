resource "null_resource" "update_jenkins_configs" {
  ...
  #SPLUNK
  provisioner "local-exec" {
    command = "${var.configureSplunk_cmd} ${lookup(var.splunkmap, "splunk_enable")} ${lookup(var.splunkmap, "splunk_endpoint")} ${lookup(var.splunkmap, "splunk_token")} ${lookup(var.splunkmap, "splunk_index")} ${aws_kinesis_stream.logs_stream_dev.arn} ${aws_kinesis_stream.logs_stream_stg.arn} ${aws_kinesis_stream.logs_stream_prod.arn} ${var.jenkinsjsonpropsfile}"
  }
  #Elasticsearch
  provisioner "local-exec" {
