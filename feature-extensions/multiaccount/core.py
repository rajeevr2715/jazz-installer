...
def deploy_core_service(args, tags):
    ...
    platform_role_arn = createplatformrole(iam_client, "%s_platform_services" % (args.jazz_stackprefix), role_document, tags)
...
def createrole(iamclient, name, role_document, tags):
    response = iamclient.create_role(
                RoleName=name,
                AssumeRolePolicyDocument=json.dumps(role_document), tags
    )
